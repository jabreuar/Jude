...
// Import dialogs
const { UserProfile } = require('./dialogs/model/userProfile');
const { WelcomeCard } = require('./dialogs/welcome');
const { GreetingDialog } = require('./dialogs/greeting');
const { TechSupportDialog } = require('./dialogs/techsupport');
const { OrderDialog } = require('./dialogs/order');
const { ServiceRequestDialog } = require('./dialogs/servicerequest');
const { WarrantyStatusDialog } = require('./dialogs/warrantystatus');

// Greeting Dialog ID
const GREETING_DIALOG = 'greetingDialog';
// Tech Support Dialog ID
const TECH_SUPPORT_DIALOG = 'techSupportDialog';
// Order Dialog ID
const ORDER_STATUS_DIALOG = 'orderDialog';
// Service request status Dialog ID
const SERVICE_REQUEST_STATUS_DIALOG = 'serviceRequestDialog'
// Service request status Dialog ID
const WARRANTY_STATUS_DIALOG = 'warrantyStatusDialog'
...
const LUIS_CONFIGURATION = 'Jude-LUIS';

// Supported LUIS Intents.
const GREETING_INTENT = 'Greetings';
const ORDER_STATUS_INTENT = 'OrderStatus';
const SERVICE_REQUEST_STATUS_INTENT = 'ServiceRequestStatus';
const TECH_SUPPORT_STATUS_INTENT = 'TechSupport';
const WARRANTY_STATUS_INTENT = 'WarrantyStatus';
...
class JudeBot {
  ...
  constructor(conversationState, userState, botConfig) {
    ...
    // Add the Greeting/TechSupportDialog/Order dialog to the set
    this.dialogs.add(new GreetingDialog(GREETING_DIALOG, this.userProfileAccessor));
    this.dialogs.add(new TechSupportDialog(TECH_SUPPORT_DIALOG, this.userProfileAccessor));
    this.dialogs.add(new OrderDialog(ORDER_STATUS_DIALOG, this.userProfileAccessor));
    this.dialogs.add(new ServiceRequestDialog(SERVICE_REQUEST_STATUS_DIALOG, this.userProfileAccessor));
    this.dialogs.add(new WarrantyStatusDialog(WARRANTY_STATUS_DIALOG, this.userProfileAccessor));
          ...
          case DialogTurnStatus.empty:
            ...
            switch (topIntent) {
              case GREETING_INTENT:
                ...
                break;
              case ORDER_STATUS_INTENT:
                await dc.beginDialog(ORDER_STATUS_DIALOG);
                break;
              case TECH_SUPPORT_STATUS_INTENT:
                await dc.beginDialog(TECH_SUPPORT_DIALOG);
                break;
              case SERVICE_REQUEST_STATUS_INTENT:
                await dc.beginDialog(SERVICE_REQUEST_STATUS_DIALOG);
                break;
              case WARRANTY_STATUS_INTENT:
                await dc.beginDialog(WARRANTY_STATUS_DIALOG);
                break;
              case NONE_INTENT:
              default:
...
module.exports.JudeBot = JudeBot;